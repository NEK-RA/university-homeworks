...
class Practice(Discipline):
  ...
  def __init__(self, name, semester, cafedra, prType, leader):
    ...
    if isinstance(prType,str) or str(prType):
      ...
      if prType.lower() in availablePr:
        ...
        raise ValueError(f"{prType} не является допустимым значением (допустимые: {availablePr})")
    else:
      raise TypeError("переданный аргумент prType невозможно преобразовать к строке")
    
    # руководитель
    if isinstance(leader,str) or str(leader):
      self.lead = str(leader)
    else:
      raise TypeError("переданный аргумент leader невозможно преобразовать к строке")
  ...
  def changeTheme(self, topic):
    if isinstance(topic,str) or str(topic):
      ...
      raise TypeError("переданный аргумент topic невозможно преобразовать к строке")
